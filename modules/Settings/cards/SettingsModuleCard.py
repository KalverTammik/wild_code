...
class SettingsModuleCard(SettingsBaseCard):
    ...
    def _build_ui(self):
        ...
        if self.module_key == Module.PROJECT.value:
            projects_temp_group, _projects_temp_button = SettingsModuleFeatureCard.build_filter_group(
                parent=cw,
                title_text=self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_HELPER_TITLE),
                lang_manager=self.lang_manager,
                description_text=self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_HELPER_DESCRIPTION),
                group_object_name="ProjectsTempLayerHelperGroup",
                container_object_name="ProjectsTempLayerHelperContainer",
                widget_factory=lambda container: self._create_projects_temp_layer_button(container),
            )
            cl.addWidget(projects_temp_group)
    ...
    def _create_projects_temp_layer_button(self, parent: QWidget) -> QPushButton:
        button = QPushButton(
            self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_CREATE_BUTTON),
            parent,
        )
        button.setObjectName("ProjectsTempLayerCreateButton")
    ...
    def _on_create_temp_projects_layer_clicked(self) -> None:
        try:
            preferred_layer = self._layer_selector.currentLayer() if self._layer_selector else None
            reference_layer = ProjectsTempLayerHelper.resolve_reference_layer(preferred_layer)
            if reference_layer is None:
                ModernMessageDialog.show_warning(
                    self.lang_manager.translate(TranslationKeys.ERROR),
                    self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_REFERENCE_REQUIRED),
                )
                return
            ...
            default_name = (
                self._pend_element_name
                or self._orig_element_name
                or ProjectsTempLayerHelper.DEFAULT_LAYER_NAME
            )
            layer_name, ok = ModernMessageDialog.get_text_modern(
                self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_PROMPT_TITLE),
                self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_PROMPT_LABEL),
                text=default_name,
            )
            if not ok:
                return
            ...
            if not normalized_name:
                ModernMessageDialog.show_warning(
                    self.lang_manager.translate(TranslationKeys.ERROR),
                    self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_NAME_REQUIRED),
                )
                return

            use_existing_label = self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_STORAGE_EXISTING)
            standalone_label = self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_STORAGE_STANDALONE)
            cancel_label = self.lang_manager.translate(TranslationKeys.CANCEL_BUTTON)
            storage_choice = ModernMessageDialog.ask_choice_modern(
                self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_STORAGE_TITLE),
                self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_STORAGE_MESSAGE),
                buttons=[use_existing_label, standalone_label, cancel_label],
                default=standalone_label,
                cancel=cancel_label,
                button_variants=[ButtonVariant.PRIMARY, ButtonVariant.SUCCESS, ButtonVariant.GHOST],
            ...
            overwrite_file = False
            if storage_choice == use_existing_label:
                if not ProjectsTempLayerHelper.gpkg_path_for_layer(reference_layer):
                    ModernMessageDialog.show_warning(
                        self.lang_manager.translate(TranslationKeys.ERROR),
                        self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_GPKG_REQUIRED),
                    )
                    return
            else:
                default_path = ProjectsTempLayerHelper.default_standalone_gpkg_path(reference_layer, normalized_name)
                selected_path, _selected_filter = QFileDialog.getSaveFileName(
                    self,
                    self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_SAVE_DIALOG_TITLE),
                    default_path,
                    "GeoPackage (*.gpkg)",
                )
                if not selected_path:
                ...
                if os.path.exists(target_gpkg_path):
                    overwrite_file = ModernMessageDialog.ask_yes_no(
                        self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_OVERWRITE_TITLE),
                        self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_OVERWRITE_MESSAGE).format(path=target_gpkg_path),
                        yes_label=self.lang_manager.translate(TranslationKeys.OK),
                        no_label=cancel_label,
                        default=self.lang_manager.translate(TranslationKeys.OK),
                    )
            ...
            created_layer, error_text = ProjectsTempLayerHelper.create_or_load_layer(
                reference_layer,
                normalized_name,
                gpkg_path=target_gpkg_path,
                overwrite_file=overwrite_file,
            ...
            if created_layer is None or not created_layer.isValid():
                ModernMessageDialog.show_warning(
                    self.lang_manager.translate(TranslationKeys.ERROR),
                    self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_CREATE_FAILED).format(
                        name=normalized_name,
                        error=error_text or self.lang_manager.translate(TranslationKeys.ERROR),
                    ),
                )
            ...
            ModernMessageDialog.show_info(
                self.lang_manager.translate(TranslationKeys.SUCCESS),
                self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_READY).format(name=actual_name),
            )
        except Exception as exc:
            PythonFailLogger.log_exception(
                exc,
                module=Module.SETTINGS.value,
                event="projects_temp_layer_button_failed",
            )
            ModernMessageDialog.show_warning(
                self.lang_manager.translate(TranslationKeys.ERROR),
                self.lang_manager.translate(TranslationKeys.PROJECTS_TEMP_LAYER_CREATE_FAILED).format(
