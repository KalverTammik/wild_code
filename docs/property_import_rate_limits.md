...
„Käivita kontroll” ja import kasutavad sama `classify_property_import()` funktsiooni. Asukohapõhise tabeli andmetes säilitatakse `muudet` väli; kontroll arvestab aadressi, backend'i katastriandmete kuupäeva ning põhikihi kuupäeva.

Olemasolu loetakse kinnistu enda `status` väljalt. Backend'i dokumentatsiooni järgi on see enum `PropertyStatus` väärtustega `ACTIVE` ja `ARCHIVED`, mis lisati 29.12.2025. Varem otsis plugin staatust eraldi kirjena mooduli staatuste hulgast; kinnistutel selliseid kirjeid ei ole, mistõttu otsing jäi tühjaks ja kood langes sildipõhisele varuteele. Nüüd küsitakse katastritunnuse kohta üks päring ja vastused jaotatakse staatuse järgi. See poolitab ka kontrolli päringute arvu. Staatuseta vanad kirjed käsitletakse endiselt arhiivisildi järgi. Arhiveerimine ja taastamine saadavad sama välja ühe muutmispäringuga, varasema kuue oletatud sisendikuju asemel.
...
Katastri aadressiväli `l_aadress` on üks tekst, backend'i sisendil on aga eraldi tänav ja majanumber. Tükeldamise reegel on järgmine. Kui aadress sisaldab `//`, läheb kogu tekst nimeväljale, sest `//` ühendab ühe objekti mitut võrdväärset aadressi ja ühte majanumbrit ei saa sealt korrektselt võtta. Muidu eraldatakse majanumbriks ainult viimane sõna ja ainult siis, kui see koosneb numbritest, millele võib järgneda üks täht ning mille sees võib olla kaldkriips või sidekriips. Kõik muu jääb nimeväljale, sealhulgas teetähised nagu `L2` ja riigitee number aadressi alguses. Ühesõnaline aadress jääb alati nimeväljale. Tühi aadress jääb tühjaks; varem võrdles kontroll neil ridadel sõna „NULL", import aga tühja sõnet.

Aadressita katastriüksus (`l_aadress` on NULL või tühi) ei ole aadressikonflikt. Import saadab siis tühja tänava, ja backend alustab liidetud rida esimesest olemasolevast asukohaosast: asustusüksusest, selle puudumisel omavalitsusest, selle puudumisel maakonnast. Kui backend'i aadressi esimene komaosa on sama asukohaosa või backend'i aadress on tühi, on kirjed kooskõlas ja otsust ei küsita. Kui backend'is on aga tänav olemas, näiteks kasutaja lisas selle käsitsi, jääb kirje otsust ootama oma põhjusega „Impordis aadress puudub, taustasüsteemis on aadress olemas”. Selle kirje puhul kustutaks „Rakenda impordi andmed” backend'i aadressi. Nagu teiste aadressierinevuste korral, rakendub uuema kuupäevaga import ilma otsuseta. „Käivita kontroll” loeb asukohaosad tabelirea impordiobjektilt, samadelt väljadelt, mida import saadab. Otsuste vaates kuvatakse põhikihi tühja aadressi kui „Puudub või teadmata”, mitte tekstina „NULL”.

Varem lõigati aadress esimese numbri kohalt, mis lõikas nime keskelt pooleks („Põhja tänav L" ja „2"), jättis riigitee nimeväljale tühja sõne ja surus `//` järel oleva teise aadressi majanumbri sisse. Võrdlus toimub kalendrikuupäevades, sest impordi API-sisend on samuti kuupäev; ajavööndiga backend'i ajatempel ei tekita enam võrreldamatute kuupäevade viga. Puuduvat või loetamatut kuupäeva ei esitata kasutajale tõendina, et import on vanem. Värske backend'i kontroll enne iga salvestamist säilib.
