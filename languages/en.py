...
TRANSLATIONS = {
    ...
    ,TranslationKeys.SELECT_SINGLE_PROPERTY_MESSAGE: "Please select only one property feature on the map."
        ...
        ,TranslationKeys.WORKS_LAYER_ID_FIELD_MISSING: "The configured Works layer is missing the task id field (`Mailabl_id` or `task_id`)."
        ...
        ,TranslationKeys.WORKS_TEMP_LAYER_HELPER_DESCRIPTION: "Temporary development helper. Creates or loads a point-based Works layer inside the same GeoPackage as the selected Works reference layer, or falls back to the configured Property main layer. The created layer is immediately configured as the active Works main layer."
        ...
        ,TranslationKeys.WORKS_TEMP_LAYER_REFERENCE_REQUIRED: "Select a reference layer for Works first, or configure the Property main layer. The helper needs an existing layer to determine which GeoPackage and CRS to use."
        ,TranslationKeys.WORKS_TEMP_LAYER_GPKG_REQUIRED: "The temporary Works layer can only be created from a GeoPackage-backed reference layer. Select a GPKG layer in the Works layer picker, or configure the Property main layer in Settings."
